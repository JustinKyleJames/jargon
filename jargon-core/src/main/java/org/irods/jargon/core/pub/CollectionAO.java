 ...
 * object, not as a {@code java.io.File} object. For familiar {@code java.io.*}
 ...
 * canonical path. For collections, AVU metadata can be different for the
 * canonical and soft linked path, and this class will use these paths as
 * explicitly provided by the caller of the metadata methods.
 *
...
public interface CollectionAO extends FileCatalogObjectAO {
	...
	/**
	 * For a given absolute path, get an {@code IRODSFileImpl} that is a collection.
	 * <p>
	 ...
	 * identifying the Collection associated with that metadata, based on a metadata
	 * query.
	 * <p>
	 * Note that this method will work across zones, so that if the given collection
	 * path is in a federated zone, the query will be made against that zone.
	 * <p>
	 * Note that for soft links, metadata is associated with the given path, so a
	...
	/**
	 * For a given iRODS collection, set the access permission to write. This can
	 * optionally be recursively applied.
	 * <p>
	 * Note that this method will work if a soft-linked collection name is supplied.
	 * Permissions are always associated with the canonical path name, and a soft
	...
	/**
	 * Check the given collection (by absolute path) to see if the inheritance flag
	 * is set. This indicates that access permissions are inherited by children of
	 * the collection.
	 * <p>
	 * Note that this method will work if a soft-linked collection name is supplied.
	 * Permissions are always associated with the canonical path name, and a soft
	 ...
	 * for a collection, and be able to easily change the value while preserving the
	 * attribute name and units. This method allows the specification of an AVU with
	 * the known name and units, and an arbitrary value. The method will find the
	 * unique attribute by name and unit, and overwrite the existing value with the
	 * value given in the {@code AvuData} parameter. *
	 * <p>
	 * Note that, in the case of a soft-linked path, the metadata is associated with
	 * that path, and is separate from metadata associated with the canonical file
	 * path
	...
	/**
	 * For a given iRODS collection,remove the access permission for the user as an
	 * administrator. This can optionally be recursively applied.
	 * <p>
	 * Note that this method will work if a soft-linked collection name is supplied.
	 * Permissions are always associated with the canonical path name, and a soft
	...
	/**
	 * For a given iRODS collection, set the default to inherit access permissions,
	 * using admin mode.
	 * <p>
	 * Note that this method will work if a soft-linked collection name is supplied.
	 * Permissions are always associated with the canonical path name, and a soft
