...
public class FederatedCollectionAndDataObjectListAndSearchAOImplTest {
	...
	/**
	 * put a collection on the federated zone, this will be accessed from a user on
	 * the primary zone this will use the fedread directory as specified in the6
	 * test-scripts/fedtestsetup_fedzonex.sh scripts.
	 */
	@Test
	public void testListCollectionWithOneDataObject() throws Exception {
		...
		String localFilePath = FileGenerator.generateFileOfFixedLengthGivenName(absPath, fileName, 3);
		...
		CollectionAndDataObjectListAndSearchAO collectionListAndSearchAO = irodsFileSystem.getIRODSAccessObjectFactory()
	...
	public void testRetrieveObjStatInFederatedZone() throws Exception {
		...
		CollectionAndDataObjectListAndSearchAO collectionListAndSearchAO = irodsFileSystem.getIRODSAccessObjectFactory()
				.getCollectionAndDataObjectListAndSearchAO(fedAccount);
		ObjStat objStat = collectionListAndSearchAO.retrieveObjectStatForPath(targetIrodsPath + "/" + fileName);
	...
	public void testListDataObjectsUnderPathInAnotherZone() throws Exception {
		...
		Assert.assertEquals("did not find all of the entries", count, entries.size());
	...
	/**
	 * Try and list stuff from the federated zone underneath the /zonename for the
	 * fed zone, should get at least the home dir
	 *
	 * @throws Exception
	 */
	@Test
	public void testListCollectionsUnderPathInAnotherZoneFromRoot() throws Exception {
		...
		Assert.assertFalse("should not have been an empty result list", entries.isEmpty());
	...
	public void testListCollectionsUnderPathNotExists() throws Exception {
		...
		IRODSAccount irodsAccount = testingPropertiesHelper
				.buildIRODSAccountForFederatedZoneFromTestProperties(testingProperties);
		String targetIrodsCollection = "/" + irodsAccount.getZone() + "/idontexistatall";
	...
	/**
	 * For a collection, add a federated user, and then query the collections with
	 * permissions to make sure the cross zone user is found, with the appropriate
	 * zone.
	 *
	 * @throws Exception
	 */
	@Test
	public void testListCollectionsUnderPathWithPermissionsIncludingACrossZoneUser() throws Exception {
		...
		collectionAO.setAccessPermissionRead("", irodsFile.getAbsolutePath(),
		...
		List<UserFilePermission> userFilePermissions = collections.get(0).getUserFilePermission();
		Assert.assertNotNull("got a null userFilePermissions", userFilePermissions);
		Assert.assertFalse("did not find the three permissions", userFilePermissions.isEmpty());
	 ...
	 * For a collection, add a federated user, and then query the collections with
	 * permissions to make sure the cross zone user is found, with the appropriate
	 * zone. In this case the query is made from a user logged in to zone 2. The
	 * query should look at the data on zone1
	 *
	 * @throws Exception
	 */
	@Ignore
	// TODO: check https://github.com/DICE-UNC/jargon/issues/127
	public void testListCollectionsUnderPathWithPermissionsIncludingACrossZoneUserQueryFromZone2() throws Exception {
		...
		IRODSAccount zone2Account = testingPropertiesHelper
				.buildIRODSAccountForFederatedZoneFromTestProperties(testingProperties);
		CollectionAndDataObjectListAndSearchAO collectionAndDataObjectListAndSearchAO = irodsFileSystem
				.getIRODSAccessObjectFactory().getCollectionAndDataObjectListAndSearchAO(zone2Account);
	...
	public void testListDataObjectsUnderPathWithAccessInfoInAnotherZone() throws Exception {
		...
		for (int i = 0; i < count; i++) {
			...
			irodsFile.createNewFile();
			irodsFile.close();
			dataObjectAO.setAccessPermissionWrite(zone1Account.getZone(), irodsFile.getAbsolutePath(),
					zone1Account.getUserName());
		}
		...
		for (CollectionAndDataObjectListingEntry resultEntry : entries) {
			Assert.assertTrue("this is not a data object",
					resultEntry.getObjectType() == CollectionAndDataObjectListingEntry.ObjectType.DATA_OBJECT);
			Assert.assertTrue("file name not correctly returned", resultEntry.getPathOrName().indexOf(fileName) > -1);
			Assert.assertTrue("did not get both expected permissions", resultEntry.getUserFilePermission().size() == 2);
			...
			Assert.assertEquals("did not set crossZone zone correctly", zone1Account.getZone(),
					crossZoneEntry.getUserZone());
			Assert.assertEquals("did not set owner zone correctly", irodsAccount.getZone(), ownerEntry.getUserZone());
	...
	@Test
	public void testCountFilesAndCollectionsUnderPathInAnotherZone() throws Exception {
	...
	/**
	 * put up a collection with some data in zone1, then list the collections and
	 * data objects from the perspective of a user on zone2. The results should be
	 * the data from zone1.
	 *
	 * @throws Exception
	 */
	@Test
	public void testListFilesAndCollectionsUnderPathWithAccessInfoInAnotherZone() throws Exception {
		...
		for (CollectionAndDataObjectListingEntry entry : entries) {
			...
			if (entry.isCollection()) {
				Assert.assertEquals("did not have the three permissions for collection", 3,
						entry.getUserFilePermission().size());
			} else {
				Assert.assertEquals(

						"did not have the  permissions for data objects", 3, entry.getUserFilePermission().size());
			}
		}
	 ...
	 */
	@Test
	public void testListDataObjectsUnderPathWhenSoftLinkInAnotherZoneBug1842() throws Exception {
		...
		CollectionAndDataObjectListAndSearchAO actual = irodsFileSystem.getIRODSAccessObjectFactory()
				.getCollectionAndDataObjectListAndSearchAO(zone1Account);
		List<CollectionAndDataObjectListingEntry> entries = actual.listDataObjectsUnderPath(softLinkCollection, 0);
		Assert.assertNotNull(entries);
		Assert.assertFalse(entries.isEmpty());
		CollectionAndDataObjectListingEntry entry = entries.get(entries.size() - 1);
		Assert.assertTrue(entry.isLastResult());
		Assert.assertEquals(entry.getCount(), entries.size());
		Assert.assertEquals("did not find all of the entries", 1, entries.size());
